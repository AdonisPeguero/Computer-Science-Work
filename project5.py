...
def getPossibleWords(wordlist, length):
    ...
    """
    List = []
    for g in range(len(wordlist)-1):
        if (len(wordlist[g]) == length):
            List.append(wordlist[g])
        elif length < 3:
             print('Number is smaller then 3')
             quit()
...
def playGame(words):
    ...
    misses = int(raw_input("how many single letter tries can you have?: "))
    guessLength = int(raw_input("Up to how many letters in one word? (no less then 3 letters) "))
    ...
    while True:
        ...
        print "Latters guessed so far:", displayGuess(guessList)
        print "Tries left:", misses
        print "Letters not used: ", lettersleft
        letter = raw_input("guess a letter (To guess the hole word please input +, you will only get one chance ): ")
        if letter == '+':
            letter = raw_input("Enter word: ")
